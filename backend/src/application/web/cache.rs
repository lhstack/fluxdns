...
use crate::dns::{CacheConfig, CacheStats};

/// Application state for cache API
#[derive(Clone)]
pub struct CacheState {
    pub business: Arc<CacheBusiness>,
}
...
///
/// GET /api/cache/stats
pub async fn cache_stats(State(state): State<CacheState>) -> Result<impl IntoResponse, ApiError> {
    Ok(Json(CacheStatsResponse::from(state.business.stats().await)))
}
...
pub async fn get_cache_config(
    ...
    Ok(Json(CacheConfigResponse::from(
        state.business.config().await,
    )))
}
...
pub async fn update_cache_config(
    ...
    let config = state
        .business
        .update_config(request.default_ttl, request.max_entries)
        .await?;
...
pub async fn clear_cache(State(state): State<CacheState>) -> Result<impl IntoResponse, ApiError> {
    state.business.clear_all().await;
...
pub async fn clear_domain_cache(
    ...
    state.business.clear_domain(&domain).await?;
...
pub async fn cleanup_cache(State(state): State<CacheState>) -> Result<impl IntoResponse, ApiError> {
    let remaining = state.business.cleanup_expired().await;
