...
pub struct StrategyState {
    pub business: Arc<StrategyBusiness>,
}
    ...
    fn from(strategy: QueryStrategy) -> Self {
        let description = match strategy {
            ...
            QueryStrategy::Fastest => {
                "基于历史响应时间选择最快的服务器（首次查询自动使用并发策略探测）"
            }
...
impl UpdateStrategyRequest {
    ...
    pub fn validate(&self) -> Result<(), ValidationErrors> {
        ...
        if !VALID_STRATEGIES.contains(&lower.as_str())
            && QueryStrategy::from_str(&self.strategy).is_none()
        {
...
pub async fn get_strategy(
    ...
    Ok(Json(StrategyResponse::from(state.business.current().await)))
}
...
pub async fn update_strategy(
    ...
    let strategy = request
        .get_strategy()
        .ok_or_else(|| AppError::Validation("Invalid strategy".to_string()))?;

    let applied = state.business.update(strategy).await?;

    Ok(Json(StrategyResponse::from(applied)))
}
...
pub async fn get_available_strategies() -> Result<impl IntoResponse, ApiError> {
    let strategies = vec![
        ...
        StrategyInfo {
            ...
            description: "基于历史响应时间选择最快的服务器（首次查询自动使用并发策略探测）"
                .to_string(),
        },
        StrategyInfo {
