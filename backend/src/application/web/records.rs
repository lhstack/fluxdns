...
use crate::infrastructure::repository::{CreateDnsRecord, DnsRecord, UpdateDnsRecord};

/// Application state for DNS records API
#[derive(Clone)]
pub struct RecordsState {
    pub business: Arc<RecordBusiness>,
}
...
pub async fn list_records(
    ...
    let records = state.business.list().await?;
...
pub async fn get_record(
    ...
    let record = state.business.get(id).await?;
...
pub async fn create_record(
    ...
    let record = state
        .business
        .create(request.into_create_dns_record())
        .await?;
...
pub async fn update_record(
    ...
    request
        .validate(&existing.record_type)
        .map_err(ApiError::validation)?;

    let record = state
        .business
        .update(id, request.into_update_dns_record())
        .await?;
    Ok(Json(RecordResponse { data: record }))
}
...
/// Build the DNS records API router
